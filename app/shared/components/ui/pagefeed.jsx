import LastSeen from "~/shared/components/logic/LastSeen";
  ...
  return (
    <div>
      {data.map((item) => (
        <div
          key={item.id}
          className="mb-4 border-b border-gray-300 py-4 pb-2 pt-4"
        >
          <h2 className="text-lg font-semibold text-black-100">
            <Link to="/comments">{item.postContent}</Link>
          </h2>
          {item.isTopComment && (
          ...
          <div className="mt-2 flex items-center">
            {item.commenter?.avatar ? (
              ...
              <div className="relative mr-2 h-10 w-10 rounded-full border-2 border-pink-300 bg-pink-100 ">
                ...
                >
                  <g clip-path="url(#clip0_307_5800)">
                    <path
      ...
      <div className="mx-auto my-10 h-[420px] w-96 bg-[url('~/assets/mobileHelpline.svg')] bg-no-repeat lg:h-[400px] lg:w-[880px] lg:bg-[url('~/assets/helpline.svg')]"></div>
    </div>
  );
}
